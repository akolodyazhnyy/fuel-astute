...
module Astute
  ...
  class UploadCirrosVMWareImage < PostDeployAction
    ...
    def process(deployment_info, context)
      ...
      if vcenter == 'vcenter'
        ...
        os['img_path'] = case controller['cobbler']['profile']
                           when 'centos-x86_64'
                             '/opt/vm/cirros-x86_64-disk.vmdk'
                           when 'rhel-x86_64'
                             '/opt/vm/cirros-x86_64-disk.vmdk'
                           when 'ubuntu_1204_x86_64'
                             '/usr/share/cirros-testvm/cirros-x86_64-disk.vmdk'
                           else
        ...
        if response[:data][:exit_code] == 0
          Astute.logger.debug "VMWare image already added to stack"
        else
          cmd = "/usr/bin/glance #{auth_params} \
                 image-create \
                   --name \'#{os['img_name']}\' \
          ...
          response = run_shell_command(context, Array(controller['uid']), cmd)
          if response[:data][:exit_code] == 0
            Astute.logger.info("#{context.task_id}: Upload VMWare cirros image is done")
          else
            msg = 'Upload VMWare cirros image failed'
